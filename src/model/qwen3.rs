/// Qwen3 dense model family (0.5B – 32B)
///
/// Key differences from Qwen2:
/// - Query-Key Normalization (QK-Norm) using RMSNorm per head
/// - Identical SwiGLU MLP
/// - Same weight layout, different config keys
///
/// Models: Qwen3-0.5B, 1.5B, 4B, 8B, 14B, 32B (instruct/thinking)
...
// RMSNorm applied per attention head for QK-Norm
struct HeadRmsNorm {
    norm: RmsNorm,
}
...
impl HeadRmsNorm {
    fn new(dim: usize, eps: f64, vb: VarBuilder) -> Result<Self> {
        let norm = RmsNorm::new(vb.get(dim, "weight")?, eps);
        Ok(Self { norm })
    }
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let (b, h, s, d) = x.dims4()?;
        // Apply RMSNorm along head_dim dimension
        let x_2d = x.reshape((b * h * s, d))?;
        let x_normed = self.norm.forward(&x_2d)?;
        x_normed.reshape((b, h, s, d))
    }
}
...
impl Qwen3Attention {
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            q_norm: HeadRmsNorm::new(head_dim, cfg.rms_norm_eps, vb.pp("q_norm"))?,
            k_norm: HeadRmsNorm::new(head_dim, cfg.rms_norm_eps, vb.pp("k_norm"))?,
...
impl Qwen3 {
    pub fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let layers = (0..cfg.num_hidden_layers)
            .map(|i| Qwen3Block::load(vb.pp(&format!("model.layers.{}", i)), cfg))
...
impl Qwen3Model {
    pub fn new(model: Qwen3, config: Config, device: Device, dtype: DType, varmap: VarMap) -> Self {
