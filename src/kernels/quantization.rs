...
pub fn dequantize_nf4(
    ...
    Err(Error::Msg("CpuFallback: dequantize_nf4 not implemented for CPU (requires cuda feature)".to_string()))
}
...
pub fn dequantize_nf4(
    ...
    match dev {
        candle_core::Device::Cuda(_) => {
            ...
            Err(Error::Msg("Not yet wired to runtime".to_string()))
        }
        _ => Err(Error::Msg("dequantize_nf4 matches cuda feature but device is not cuda".to_string()))
    }
}
